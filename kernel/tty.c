...
PUBLIC void in_process(TTY* tty, u32 key)
{
	if (!(key & FLAG_EXT)) {
			...
			if ((key & FLAG_SHIFT_L) || (key & FLAG_SHIFT_R)) /* Shift + Up */
			{
				scroll_screen(tty->console, SCR_DN);
                        ...
                        else /* Up */
                        {
                            ...
                            {
                                tty->show--;
                                putstring(tty,tty->inputs[tty->show]);
                            }
                            if(tty->show == 0 && strlen(tty->inputs[15]) > 0) //circle 
			    {
                                tty->show = 15;
                                putstring(tty,tty->inputs[tty->show]);
                            }
		...
		case DOWN:
			...
			if(tty->show < tty->latestinput)
                        {
                            tty->show++;
                            putstring(tty,tty->inputs[tty->show]);
                        }
                        else if(tty->show == 15 && tty->show > tty->latestinput) //circle 
			{
                            tty->show = 0;
                            putstring(tty,tty->inputs[tty->show]);
                        }
...
PRIVATE void putstring(TTY *tty, const char* string)
{
    empty(tty);
    tty->index = strlen(string);
    for(int i=0;i<strlen(string);i++)
    {
        put_key(tty,string[i]);
        tty->inputing[i] = string[i];
    }
}

PRIVATE void remenber(TTY *tty)
{
    strcpy(tty->inputs[tty->latestinput],tty->inputing);   //push your input into tty->inputs

    tty->latestinput++;                           //index move forward
    if(tty->latestinput == 16)
    {
        tty->latestinput = 0;                     //fill tty->inputs, go back to begin
    }
    tty->show = tty->latestinput;           //always show the latest input 
    while(tty->index > 0)                   //set index to 0
    {
	tty->inputing[tty->index] = 0;
        tty->index--;
    }
    tty->inputing[tty->index] = 0;          //reset tty->inputing
}
...
PRIVATE void match(TTY* tty)
{
    ...
    if(tty->inputing[startpoint] == ' ' && startpoint != 0)   //match filename
    {
    	...
    	step = 0;
    	len = 0;
        if(filepath[step] == tty->inputing[startpoint])
        {
